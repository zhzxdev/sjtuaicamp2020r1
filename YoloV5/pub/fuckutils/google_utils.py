...
def attempt_download(weights):
    ...
    if len(weights) > 0 and not os.path.isfile(weights):
        d = {
            'yolov3-spp.pt': '1mM67oNw4fZoIOL1c8M3hHmj66d8e-ni_',  # yolov3-spp.yaml
...
def gdrive_download(id='1n_oKgR81BJtqk75b00eAjdv03qVCQn2f', name='coco128.zip'):
    ...
    if os.path.exists('cookie'):  # large file
        s = "curl -Lb ./cookie \"drive.google.com/uc?export=download&confirm=`awk '/download/ {print $NF}' ./cookie`&id=%s\" -o %s" % (id, name)
    else:  # small file
